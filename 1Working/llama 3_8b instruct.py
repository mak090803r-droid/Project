"""
llama 3_8b instruct.py
======================
Integrated OCR → Translation → TTS pipeline.
Uses Meta Llama 3 8B Instruct (via HuggingFace Transformers) for translation.

Usage:
    python "llama 3_8b instruct.py"
...
# Official gated model — swap to NousResearch mirror if no HF access
#MODEL_ID = "meta-llama/Meta-Llama-3-8B-Instruct"
MODEL_ID = "NousResearch/Meta-Llama-3-8B-Instruct"

# Generation settings for translation (low temperature = deterministic)
# Built as a GenerationConfig object to suppress transformers warnings:
#   - pad_token_id set explicitly (= eos_token_id 128001 for Llama 3) so
#     transformers never auto-infers it and prints a log line
#   - max_length=None overrides the HF pipeline's internal default of 4096
#     so there is no conflict with max_new_tokens
from transformers import GenerationConfig as _GenCfg
GEN_CONFIG = _GenCfg(
    max_new_tokens=512,
    max_length=None,          # override pipeline default (4096) — no conflict
    pad_token_id=128001,      # Llama 3 eos_token_id — silences pad_token warning
    temperature=0.3,
    top_p=0.9,
    do_sample=True,
...
def load_llama_translator(language: str):
    """
    Load Llama 3 8B Instruct model via HuggingFace Transformers.
    Returns (pipeline) or None for English (no translation needed).
    Auto-applies 4-bit / 8-bit quantisation based on available VRAM.
    """
    if language == "english":
    ...
    print(f"[LOAD] Loading Llama 3 8B Instruct translator …")
    t = time.time()

    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID)
    tokenizer.pad_token = tokenizer.eos_token

    # Auto-select quantisation based on available VRAM
    bnb_config = None
    if torch.cuda.is_available():
        vram_gb = torch.cuda.get_device_properties(0).total_memory / 1e9
        if vram_gb < 10:
            print("[INFO] Low VRAM — enabling 4-bit quantisation (bitsandbytes)")
            ...
            print("[INFO] Medium VRAM — enabling 8-bit quantisation (bitsandbytes)")
    ...
    print(f"[LOAD] Llama 3 8B Instruct translator ready in {time.time()-t:.2f}s")
    return pipe
...
def run_translation_llama(text: str, language: str, pipe) -> tuple[str, float]:
    """
    Translate text from source language → English using Llama 3 8B Instruct.
    ...
    src_lang_name = LANGUAGE_NAMES.get(language, language.capitalize())
    print(f"[STAGE 2] Translating to English (Llama 3 8B Instruct) …")
    t0 = time.time()
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Your task is to translate text accurately into English. "
                "Output ONLY the translated English text — no explanations, "
                "no notes, no extra commentary."
            ),
        },
        {
            "role": "user",
            "content": (
                f"Translate the following {src_lang_name} text to English:\n\n"
                f"{text}"
            ),
        },
    ]
    ...
    if isinstance(full_output, list):
        # Chat-format — last dict is the assistant reply
        translated = full_output[-1]["content"].strip()
    else:
...
║       Smart Glasses FYDP — Llama 3 8B Instruct Edition      ║
╚══════════════════════════════════════════════════════════════╝
"""
...
def main():
    ...
    print(f"  Translator : Llama 3 8B Instruct (HuggingFace Transformers)")
    print("=" * 60)
    ...
    llama_pipe  = load_llama_translator(language)
    tts_module  = load_tts()
    ...
    while True:
        ...
        # Stage 2: Translation (Llama 3 8B Instruct)
        english_text, t_translate = run_translation_llama(extracted_text, language, llama_pipe)

        if language != "english":
            print(f"\n{'='*60}")
            print("  TRANSLATED TEXT (English) — Llama 3 8B Instruct")
            print(f"{'='*60}")
            print(english_text)
            print(f"{'='*60}\n")
        ...
        # ── Timing Summary ─────────────────────────────────────────
        print(f"\n{'─'*40}")
        print("  ⏱  TIMING BREAKDOWN (Llama 3 8B Instruct)")
        print(f"{'─'*40}")
