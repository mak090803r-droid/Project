...
## 8. Development Constraints for Codex
*   **Original Code Preservation:** Keep original files in `1Pipeline/` and `1Working/` untouched. Always perform development modifications on the files inside `1Pipeline/final/`.
*   **Execution Logs:** All scripts must run cleanly without socket crashes. Ensure exceptions are wrapped with safe disconnect hooks to prevent socket binding lockups upon exit.
