"""
pipeline_cli_nocam.py  (Final Demo — No Camera Receiver)
=========================================================
PC-side TCP server that receives static images sent from the Pi client
(piweb_cli_nocam.py) over TCP port 9999.
No continuous video stream preview is shown. It just stands by,
receives images on demand, and runs OCR → translation → TTS.

Demo Flow:
  1. TTS "Welcome to FYDP Glasses"
  2. CLI: Choose language + book mode
  3. Load models → TTS "Models loaded"
  4. Wait for Pi client to connect
  5. Wait for Pi to send a frame (entered via path on the Pi terminal)
  6. Process: OCR → translate → speak
  7. During TTS: press any key to stop

Usage:
    python pipeline_cli_nocam.py
"""
...
# ══════════════════════════════════════════════════════════════════════════════
#  WIRE PROTOCOL — shared with piweb_cli_nocam.py
# ══════════════════════════════════════════════════════════════════════════════
MSG_JSON  = 0x01
MSG_FRAME = 0x02
...
# ══════════════════════════════════════════════════════════════════════════════
#  AUDIO TONES  (played on PC speakers)
# ══════════════════════════════════════════════════════════════════════════════
...
def load_ocr_engine():
    ...
    print(f"[LOAD] PaddleOCR (onnxruntime, {ocr_device}) …")
    t = time.time()
    ocr = PaddleOCR(
...
def load_nllb_translator(language):
    if language == "english":
        return None, None
    print("[LOAD] NLLB-200 1.3B Distilled …")
    t = time.time()
    if not os.path.exists(NLLB_MODEL_DIR):
        print(f"[INFO] Compiling {NLLB_BASE_MODEL} …")
        cmd = (
...
def save_frame(frame, run_count):
    ...
    filename  = f"capture_nocam_{run_count:03d}_{timestamp}.png"
...
def run_translation_nllb(text, language, translator, tokenizer):
    if translator is None or tokenizer is None:
        print("[STAGE 2] Skipping translation (English)")
        return text, 0.0
...
def run_tts(pipertts_module, text):
    """Speak text. Press any key in terminal to stop."""
...
def start_server(port):
    ...
    print("  Waiting for Pi Client to connect …\n")
    client, addr = server.accept()
    print(f"[SERVER] ✅ Connected to Client from {addr}")
...
BANNER = r"""
╔══════════════════════════════════════════════════════════════╗
║   FYDP SMART GLASSES — RECEIVER PIPELINE (CLI, NO CAMERA)   ║
║          OCR → TRANSLATE (NLLB 1.3B) → SPEAK               ║
║                                                              ║
║  Standing by to receive static images from piweb_cli_nocam  ║
╚══════════════════════════════════════════════════════════════╝
"""
...
def main():
    ...
    # ── 2. Load heavy base models while welcome plays ──
    ...
    # Disable unwarper if book mode is off
    if not book_mode:
        unwarper = None

    # Load translation model for selected language
    print(f"\n[INFO] Loading NLLB Translator for {language.upper()} …")
    with SuppressOutput():
        translator, tokenizer = load_nllb_translator(language)

    tts_module.speak("Models loaded. Waiting for network connection.")
    tts_module.wait_until_done()

    # ── 4. Connect ──
    server_sock, client_sock = start_server(PORT)
    tts_module.speak("Smart glasses connected. Ready.")
    tts_module.wait_until_done()

    # ── 5. Image receiving loop ──
    run_count = 0

    try:
        while True:
            print("\n⏳ Waiting for image from Pi client …")
            ...
            if msg_type == MSG_FRAME:
                ...
                run_count += 1
                print(f"\n{'─'*60}")
                print(f"  RUN #{run_count}  |  Received from client")
                print(f"{'─'*60}\n")
