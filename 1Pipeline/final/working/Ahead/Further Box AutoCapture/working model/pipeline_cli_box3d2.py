"""
pipeline_cli_box3d2.py  (Box3d1 + Paragraph Detection Fix)
==================================================================================
Based exactly on pipeline_cli_box3d1.py. Only paragraph detection and its debug reporting are changed:
  ...
  6. All non-paragraph behaviour from box3d1 is preserved unchanged.

Usage:
    C:/Users/ali/Desktop/FYDP/fydp/Scripts/python.exe pipeline_cli_box3d2.py
"""
...
def group_ocr_lines_into_paragraphs(lines, image_shape):
    ...
    def is_visual_heading(index):
        line = ordered[index]
        text = line["text"].strip()
        if explicit_heading(line):
            return True
        if not text or len(text) > 120 or ends_sentence(text):
            return False
        ...
        # left-aligned titles in the multilingual test sheets.
        if index == 0 and pitch_after(index) >= 1.15 * typical_pitch:
            return (
                (centred and shortish) or title_like or
                line["width"] <= 0.72 * median_width)
        return (
            centred and shortish and
    ...
    heading_flags = [
        is_visual_heading(index) for index in range(len(ordered))]
...
BANNER = r"""
╔══════════════════════════════════════════════════════════════╗
║   FYDP SMART GLASSES — BOX3D2 PARAGRAPH PIPELINE          ║
