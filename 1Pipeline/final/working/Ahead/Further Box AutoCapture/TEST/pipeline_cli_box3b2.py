"""
pipeline_cli_box3b2.py  (box3b + SymSpell + Regex Spell Correction)
====================================================================
Copy of pipeline_cli_box3b.py with a lightweight, zero-GPU spell
correction stage using SymSpell (edit-distance dictionary lookup)
and hand-crafted regex rules for common OCR error patterns.

No additional ML model needed — runs in <10ms.

Usage:
    python pipeline_cli_box3b2.py
"""
...
# ══════════════════════════════════════════════════════════════════════════════
#  SPELL CORRECTION — SymSpell + Regex (no GPU, <10ms)
# ══════════════════════════════════════════════════════════════════════════════

# Technical terms OCR often mangles — protect them from SymSpell
_PROTECTED_TERMS = {
    # Abbreviation base list (additional to the generic uppercase check)
    "MOSFET", "GNSS", "CLAHE", "OFDM", "UAS", "UAV", "LIDAR", "LiDAR",
    "IMU", "GPS", "INS", "MEMS", "GVT", "FMEA", "MTBF", "PWM", "PCB",
    "UART", "SPI", "I2C", "CAN", "TCP", "UDP", "RTSP", "JPEG", "ONNX",
    "NLLB", "TTS", "OCR", "PaddleOCR", "CTranslate2", "CUDA", "GPU",
    "RTX", "VRAM", "HUD", "OSD", "FPV", "ESC", "BEC", "PID",
    "ADS-B", "TCAS", "ACAS", "RPAS", "BVLOS", "VTOL", "STOL",
    "dBm", "GHz", "MHz", "kHz", "mAh", "LiPo", "NiMH",
    
    # Technical words to protect from dictionary replacements
    "brushless", "powertrain", "telemetry", "unwarping", "unwarper", "unwarped",
    "autofocus", "dynamometer", "multilingual", "transmitters", "barometers", "barometer",
    "fuselage", "topologically", "micro-lighting", "evenness", "aerodynamic", "structural"
}

# Regex to match brackets:
# Pattern 1 (standard brackets): [o2], (1S), |01)
_STANDARD_BRACKETS = re.compile(
    r'[\[\(\|]\s*([oO0-9sS]{1,3})\s*[\]\)\|]'
)
# Pattern 2 (letter-based brackets): l01], I02), j03)
_LETTER_BRACKETS = re.compile(
    r'(?<!\w)[jJlI]\s*([oO0-9]{1,3})\s*[\]\)\|jJ]'
)
# Pattern 3 (half brackets): [o1 , |02 ) but only standard bracket symbols to avoid word clashes
_HALF_BRACKETS = re.compile(
    r'(?<!\w)[\[\(\|]\s*([oO0-9sS]{1,3})\b(?![\]\)\|])'
)


def load_spell_corrector():
    """Load SymSpell with the built-in English frequency dictionary."""
    import importlib.resources
    t0 = time.time()
    sym = SymSpell(max_dictionary_edit_distance=2, prefix_length=7)
    # Use the bundled frequency dictionary
    import pkg_resources
    dict_path = pkg_resources.resource_filename(
        "symspellpy", "frequency_dictionary_en_82_765.txt"
    )
    sym.load_dictionary(dict_path, term_index=0, count_index=1)
    # Also load bigram dictionary for compound splitting
    bigram_path = pkg_resources.resource_filename(
        "symspellpy", "frequency_bigramdictionary_en_243_342.txt"
    )
    if os.path.exists(bigram_path):
        sym.load_bigram_dictionary(bigram_path, term_index=0, count_index=2)
    print(f"[LOAD] SymSpell dictionary loaded in {time.time()-t0:.2f}s")
    return sym


def _fix_brackets(text):
    """Fix OCR bracket mangling generically: j01) → [01], [o1 → [01], etc."""
    trans = str.maketrans('oOsS', '0055')
    def _repl(m):
        raw_num = m.group(1)
        clean_num = raw_num.translate(trans)
        return f"[{clean_num}]"
    
    text = _STANDARD_BRACKETS.sub(_repl, text)
    text = _LETTER_BRACKETS.sub(_repl, text)
    text = _HALF_BRACKETS.sub(_repl, text)
    return text


def _fix_known_ocr_swaps(text):
    """Fix deterministic single-char OCR swaps seen repeatedly."""
    # Common OCR confusions (case-aware)
    swaps = [
        # Lowercase confusions
        (r'\bintemal\b',       'internal'),
        (r'\bsiructural\b',    'structural'),
        (r'\bprionitize\b',    'prioritize'),
        (r'\beerodynamic\b',   'aerodynamic'),
        (r'\bempiy-weighi\b',  'empty-weight'),
        (r'\baircrah\b',       'aircraft'),
        (r'\bstould\b',        'should'),
        (r'\bnarduare\b',      'hardware'),
        (r'\bempfoy\b',        'employ'),
        (r'\breaasal\b',       'triaxial'),
        (r'\bmagretometers\b', 'magnetometers'),
        (r'\btemomagnetic\b',  'ferromagnetic'),
        (r'\boternding\b',     'overriding'),
        (r'\botetacle\b',      'obstacle'),
        (r'\bejnanic\b',       'dynamic'),
        (r'\bthughest\b',      'highest'),
        (r'\bimanimizing\b',   'minimizing'),
        (r'\bnoer\b',          'power'),
        (r'\bconsumpion\b',    'consumption'),
        (r'\bemployrng\b',     'employing'),
        (r'\baspers-atio\b',   'aspect-ratio'),
        (r'\bgeametries\b',    'geometries'),
        (r'\bdrely\b',         'directly'),
        (r'\bgovem\b',         'govern'),
        (r'\brelaility\b',     'reliability'),
        (r'\breal-tme\b',      'real-time'),
        (r'\bpowertran\b',     'powertrain'),
        (r'\bconigurations\b', 'configurations'),
        # Uppercase / acronym confusions
        (r'\bGinsS\b',         'GNSS'),
        (r'\bOF DM\b',         'OFDM'),
    ]
...
def run_spell_correction(sym_spell, text):
    """
    Correct OCR errors using SymSpell + regex rules.
    Pipeline:
      1. Fix bracket patterns (deterministic regex)
      2. Fix known OCR swap patterns (deterministic regex)
      3. Run SymSpell word-by-word on non-protected words
    """
    if not text or not text.strip():
        return text

    # Step 1: Fix brackets
    text = _fix_brackets(text)

    # Step 2: Fix known OCR swaps
    text = _fix_known_ocr_swaps(text)

    # Step 3: SymSpell word-by-word correction
    words = text.split()
    corrected_words = []
    for word in words:
        # Strip trailing punctuation for lookup, re-attach after
        stripped = word.rstrip('.,;:!?"\')]}"')
        punct_tail = word[len(stripped):]
        core = stripped.lstrip('([{"\'')
        punct_head = stripped[:len(stripped)-len(core)] if core != stripped else ''

        # Skip protected terms (specific technical lowercase words not in dict)
        if core.lower() in _PROTECTED_TERMS:
            corrected_words.append(word)
            continue

        # Skip numbers or words containing digits (e.g. T700, 7075)
        ...
        # Skip hyphenated words generically (keeps real-time, Li-Ion, etc.)
        if '-' in core:
            corrected_words.append(word)
            continue

        # Skip acronyms generically (all-uppercase, including plurals like ESCs, IMUs)
        core_clean = core.rstrip('sS')
        if core_clean.isupper() and len(core_clean) >= 2:
            corrected_words.append(word)
            continue

        suggestions = sym_spell.lookup(
            core.lower(), Verbosity.CLOSEST,
            max_edit_distance=2,
        ...
        if suggestions and suggestions[0].distance > 0:
            ...
            corrected_words.append(f"{punct_head}{replacement}{punct_tail}")
        else:
            corrected_words.append(word)
...
def process_and_speak(frame, run_count, language, book_mode,
    ...
    try:
        ...
        with open(log_path, "a", encoding="utf-8") as f_log:
            ...
            f_log.write(f"CODE VERSION: pipeline_cli_box3b2.py\n")
    ...
    # Stage 2.5: SymSpell + Regex Spell Correction (on English text)
    t_spell = 0.0
    if spell_corrector is not None:
        print("[STAGE 2.5] Running SymSpell + regex spell correction …")
...
BANNER = r"""
╔══════════════════════════════════════════════════════════════╗
║  FYDP SMART GLASSES — BOX3B2 (SymSpell + Regex)              ║
║  OCR → TRANSLATE (NLLB 1.3B) → SYMSPELL FIX → SPEAK        ║
║                                                              ║
║  Controls:                                                   ║
...
def main():
    ...
    print("[INFO] Loading SymSpell dictionary …")
