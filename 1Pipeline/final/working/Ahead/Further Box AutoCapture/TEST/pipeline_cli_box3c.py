"""
pipeline_cli_box3c.py  (Box3b + Conservative SymSpell OCR Correction)
==================================================================================
...
Usage:
    C:/Users/ali/Desktop/FYDP/fydp/Scripts/python.exe pipeline_cli_box3c.py
"""
...
def process_and_speak(frame, run_count, language, book_mode,
    ...
    # Stage 2.5: conservative SymSpell correction on the final English text.
    # The raw OCR/translation remains available for comparison and logging.
    corrected_text = english_text
    spell_changes = []
    t_spell = 0.0
    if SPELL_CORRECTOR is not None:
        print("[STAGE 2.5] Running conservative SymSpell correction …")
        spell_t0 = time.time()
        corrected_text, spell_changes = run_spell_correction(
            SPELL_CORRECTOR, english_text)
        t_spell = time.time() - spell_t0
        if spell_changes:
            print(
                f"[STAGE 2.5] Applied {len(spell_changes)} high-confidence "
                f"correction(s) in {t_spell:.3f}s:")
            for original, replacement in spell_changes:
                print(f"  {original} -> {replacement}")
            print(f"\n{'='*60}")
            print("  CONSERVATIVELY CORRECTED TEXT")
            print(f"{'='*60}")
    ...
    try:
        ...
        with open(log_path, "a", encoding="utf-8") as f_log:
            ...
            f_log.write(f"CODE VERSION: pipeline_cli_box3c.py\n")
            ...
            if spell_changes:
                ...
                f_log.write("CORRECTIONS:\n")
                for original, replacement in spell_changes:
                    f_log.write(f"  {original} -> {replacement}\n")
            f_log.write("="*80 + "\n\n")
    except Exception as e_log:
        print(f"[LOG ERROR] Failed to save OCR output to log: {e_log}")

    # Stage 3: TTS
    t_tts = run_tts(tts_module, corrected_text, key_queue, event_queue)
...
BANNER = r"""
╔══════════════════════════════════════════════════════════════╗
║   FYDP SMART GLASSES — FINAL DEMO PIPELINE (CLI + BOX v3)  ║
