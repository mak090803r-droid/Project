...
def group_ocr_lines_into_paragraphs(lines, image_shape):
    ...
    [BOX3D1] Simplified: paragraphs are separated by a blank line in the
    document.  The gap threshold is set to catch blank-line spacing
    (typical_gap + 0.5 * median_height) which is well above normal inter-line
    gaps but well below two blank lines.  Headings, section markers, and
    chapter titles still trigger paragraph breaks independently.
    """
    if not lines:
        return []
    ...
    # [BOX3D1] Blank-line detection: a blank line creates a gap of roughly
    # typical_gap + median_height.  This threshold sits halfway, catching
    # blank lines while ignoring normal inter-line variation.
    paragraph_gap_limit = typical_gap + 0.5 * median_height
