"""
pipeline_cli_box4a.py  (Box4 + persistent live debugging)
==================================================================================
...
preserved. Box4 replaces only the pre-capture analysis/guidance subsystem:
...
Usage:
    C:/Users/ali/Desktop/FYDP/fydp/Scripts/python.exe pipeline_cli_box4a.py
"""
...
class DebugSessionRecorder:
    ...
    def submit_live_frame(self, frame, sequence, frame_timestamp):
        ...
        if self._video_queue.full():
            ...
            return
        item = (frame.copy(), int(sequence), float(frame_timestamp), time.time())
        try:
    ...
    def record_analysis(self, payload, assessment, state, passed,
        ...
        try:
            self._artifact_queue.put_nowait((
                ...
                payload["frame"].copy(), DEBUG_RAW_JPEG_QUALITY))
        except queue.Full:
...
GUIDANCE_PROMPTS = {
    "page_not_found": "Point the camera toward the page.",
    "look_up": "Look up a little.",
    "look_down": "Look down a little.",
    "look_left": "Look left a little.",
    "look_right": "Look right a little.",
    "move_closer": "Move closer a little.",
    ...
    "almost_ready": "Good position. Hold still.",
}
...
class FrameHolder:
    ...
    def get_snapshot(self):
        ...
        with self._lock:
            ...
            # iteration; the analysis launch makes its one required copy.
...
# Box4 capture analysis is resolution-normalized and calibrated on the supplied
...
CAPTURE_TEXT_MARGIN_X = 0.040
CAPTURE_TEXT_MARGIN_Y = 0.060
CAPTURE_PAPER_BORDER_FRACTION = 0.20
CAPTURE_PAPER_BORDER_RUN = 0.20
# Empirically calibrated on the 34 saved 1080p captures and 25 head-mounted
# stills.  These describe dense printed-content completeness; they are kept
# separate from physical paper-edge visibility because blank A4 margins can be
# outside the frame while every printed line is still present.
CAPTURE_CONTENT_MIN_ROWS = 24
CAPTURE_SPARSE_MAX_ROWS = 12
CAPTURE_CONTENT_LIGHT_STD_MAX = 12.93
CAPTURE_CONTENT_WIDTH_MAX = 0.4052
CAPTURE_CONTENT_BACKGROUND_MAX = 200.2
CAPTURE_CONTENT_LEFT_MARGIN_FALLBACK = 0.3229
CAPTURE_DIRECTION_HARD_EDGE = 0.012
CAPTURE_DIRECTION_OPPOSITE_MARGIN = 0.18
CAPTURE_OUTSIDE_TEXT_MEAN_MIN = 3.60
CAPTURE_OUTSIDE_TEXT_FRACTION_MIN = 0.080
# Inter-sample motion is measured 0.70 seconds apart.  Normal head-mounted
...
# gates below.
CAPTURE_MOTION_SHIFT_MAX = 0.045
CAPTURE_MOTION_GEOMETRY_MAX = 0.0525
CAPTURE_MOTION_AREA_CHANGE_MAX = 0.27
CAPTURE_MOTION_ANGLE_MAX = 6.0
...
def _outside_ocr_text_evidence(gray, text_rect):
    ...
    stroke_fraction = float(np.mean(blackhat >= 12))
    return {
...
def analyze_capture_frame(frame, ocr_engine):
    ...
    base = {
        ...
        "unreadable_sides": [],
        "outside_text_below": {
            "height": 0, "mean": 0.0, "fraction": 0.0, "present": False},
        "boxes": [],
        "outer_box": None,
        "paper": None,
        "row_count": 0,
    ...
    outside_text_below = _outside_ocr_text_evidence(original_gray, rect_orig)
    unreadable_sides = ["B"] if outside_text_below["present"] else []
    text_margins = {
        "L": x1 / w, "T": y1 / h,
        "R": (w - x2) / w, "B": (h - y2) / h,
    }
    ...
    physical_sides = []
    for side in "LTRB":
        margin_limit = CAPTURE_TEXT_MARGIN_X if side in "LR" else CAPTURE_TEXT_MARGIN_Y
        if text_margins[side] <= margin_limit:
            text_edge_sides.append(side)
        if paper is not None:
            physical_edge = (
                paper["fractions"][side] >= CAPTURE_PAPER_BORDER_FRACTION
                and paper["runs"][side] >= CAPTURE_PAPER_BORDER_RUN)
            if physical_edge:
                physical_sides.append(side)
    hard_text_edge_sides = [
        side for side in "LTRB"
        if text_margins[side] <= CAPTURE_DIRECTION_HARD_EDGE
    ]
    # A rotated but fully visible title can make its detection polygon touch
    # y=0. Bottom/side contact is much more directly associated with lost body
    # text in the calibration set, so top contact remains guidance evidence but
    # is not by itself a dense-content veto.
    hard_content_cut_sides = [
        side for side in hard_text_edge_sides if side in "LRB"
    ]
    ...
    # Dense pages need evidence that distinguishes a complete block of printed
    # content from a crisp but clipped block.  OCR confidence cannot do that:
    # incomplete calibration captures frequently scored above 0.97 confidence.
    dense_content_complete = (
        row_count >= CAPTURE_CONTENT_MIN_ROWS
        and quality["light_tile_std"] <= CAPTURE_CONTENT_LIGHT_STD_MAX
        and cluster_width_ratio <= CAPTURE_CONTENT_WIDTH_MAX
        and not hard_content_cut_sides
        and not unreadable_sides
        and (
            cluster["background"] <= CAPTURE_CONTENT_BACKGROUND_MAX
            or text_margins["L"] > CAPTURE_CONTENT_LEFT_MARGIN_FALLBACK
        )
    )

    # A genuinely sparse page cannot satisfy a dense-row rule.  It is accepted
    # only when the strict paper component is wholly inside the image and the
    # detected text itself is clear of every frame edge.  Dense pages never use
    # this fallback, which prevents the historical partial-page false captures.
    sparse_physical_complete = (
        3 <= row_count <= CAPTURE_SPARSE_MAX_ROWS
        and physical_page_safe
        and not text_edge_sides
        and not unreadable_sides
    )
    content_complete = dense_content_complete or sparse_physical_complete
    page_found = len(cluster["boxes"]) >= 3
    page_complete = page_found and content_complete

    # Direction is reported only after content is judged incomplete.  Prefer a
    # single obvious missing side; otherwise retain all corroborated/physical
    # sides so guidance safely says "move back" instead of inventing a turn.
    missing_sides = []
    if not content_complete:
        hard_direction = []
        if (text_margins["T"] <= CAPTURE_DIRECTION_HARD_EDGE
                and text_margins["B"] >= CAPTURE_DIRECTION_OPPOSITE_MARGIN):
            hard_direction.append("T")
        if (text_margins["B"] <= CAPTURE_DIRECTION_HARD_EDGE
                and text_margins["T"] >= CAPTURE_DIRECTION_OPPOSITE_MARGIN):
            hard_direction.append("B")
        if (text_margins["L"] <= CAPTURE_DIRECTION_HARD_EDGE
                and text_margins["R"] >= CAPTURE_DIRECTION_OPPOSITE_MARGIN):
            hard_direction.append("L")
        if (text_margins["R"] <= CAPTURE_DIRECTION_HARD_EDGE
                and text_margins["L"] >= CAPTURE_DIRECTION_OPPOSITE_MARGIN):
            hard_direction.append("R")
        if hard_direction:
            missing_sides = hard_direction
        else:
            corroborated = [
                side for side in text_edge_sides if side in physical_sides
            ]
            if corroborated:
                missing_sides = corroborated
            elif len(physical_sides) == 1:
                missing_sides = list(physical_sides)
            elif physical_sides:
                missing_sides = list(physical_sides)
            elif len(text_edge_sides) == 1:
                missing_sides = list(text_edge_sides)
            elif text_edge_sides:
                missing_sides = list(text_edge_sides)
    ...
    median_line_ratio = cluster["median_height"] / h
    too_far = (page_complete and page_long_ratio > 0.0
               and page_long_ratio < 0.52 and median_line_ratio < 0.012)
    ...
    base.update({
        ...
        "too_far": too_far,
        "text_readable": len(cluster["boxes"]) >= 3,
        "missing_sides": missing_sides,
        "text_edge_sides": text_edge_sides,
        "physical_sides": physical_sides,
        "hard_text_edge_sides": hard_text_edge_sides,
...
def detect_text_region_size(frame, ocr_engine):
    """Compatibility wrapper backed by Box4's synchronized page analysis."""
...
def draw_quality_overlay(frame, assessment=None, stability_count=0):
    """Draw only the dominant document cluster and Box4 gate decisions."""
    display = frame.copy()
    h, w = display.shape[:2]
    if assessment is not None:
        ...
        if outer is not None:
            paper_hull = outer.get("paper_hull")
            if paper_hull is not None:
                cv2.polylines(
                    display, [paper_hull], isClosed=True,
                    color=(0, 215, 255), thickness=2)
            x1, y1, x2, y2 = outer["rect"]
            page_ok = bool(assessment.get("page_complete"))
            box_color = (0, 220, 0) if page_ok else (0, 0, 230)
            sides = "".join(assessment.get("missing_sides", []))
            unreadable = "".join(assessment.get("unreadable_sides", []))
            label = "CONTENT SAFE" if page_ok else (
                f"TEXT SOFT: {unreadable}" if unreadable else
                f"EDGE UNSAFE: {sides}" if sides else "PAGE NOT CONFIRMED")
    ...
    if assessment is None:
        ...
        details = (
            f"Rows:{assessment.get('row_count', 0)}  "
...
def _estimate_capture_motion(previous_payload, current_payload):
    ...
    return {
        ...
        "ok": (
            shift_ratio <= CAPTURE_MOTION_SHIFT_MAX
            and geometry_shift <= CAPTURE_MOTION_GEOMETRY_MAX
            and area_change <= CAPTURE_MOTION_AREA_CHANGE_MAX
            and angle_change <= CAPTURE_MOTION_ANGLE_MAX
        ),
        "score": float(score),
        "shift": shift_ratio,
...
def _guidance_for_assessment(assessment):
    ...
    if not assessment.get("page_found", False):
        if motion_known and not motion_ok:
            return "hold_still"
        if not assessment.get("focus_ok", False):
            return "blurry"
    ...
    # The page can be fully in view while a soft lower paragraph produces no
    # OCR boxes.  That is a readability failure, not a request to move/turn.
...
def pre_capture_quality_loop(frame_holder, ocr_engine, tts_module,
    ...
    print("\n[QUALITY] Box4 head-mounted capture guidance active …")
    print("[QUALITY] Checking document coverage, page-ROI focus/light, and motion.")
    print("[QUALITY] Three distinct analyzed frames are required. S still force-captures.")

    guidance = None
    try:
        guidance = AudioGuidance(
            tts_module, state_cooldown=1.8, global_cooldown=0.70,
            event_callback=(
        ...
        while True:
            ...
            if (sequence != last_started_sequence
                ...
                threading.Thread(
                    ...
                    args=(frame.copy(), sequence, frame_timestamp,
            ...
            if (payload is not None
                ...
                if assessment is None:
                    ...
                    if passed:
                        ...
                        stable_payloads = stable_payloads[-CAPTURE_REQUIRED_DISTINCT:]
                    else:
                        if stable_payloads:
                            print("  [STABLE HOLD] Reset by a fresh failed observation.")
                        stable_payloads = []
                    ...
                    if (len(stable_payloads) >= CAPTURE_REQUIRED_DISTINCT
                        ...
                        chosen = max(
                            ...
                            key=lambda item: (
                                ...
                                + 0.25 * item["assessment"].get(
                                    "line_lap_p20", 0.0)))
                        print(
                            "\n[QUALITY] ✅ Three distinct stable observations passed; "
                ...
                if state == pending_guidance:
                    ...
                    pending_guidance_count = 1
                if guidance is not None and pending_guidance_count >= 2:
                    guidance.notify(state)

            display = draw_quality_overlay(
                frame, latest_assessment, len(stable_payloads))
            cv2.imshow(
                ...
                while not key_queue.empty():
                    ...
                    if key == 's':
                        print(
                            ...
                            "all Box4 safety gates were bypassed.")
                        _safe_debug_call(
                            debug_recorder, "record_event",
...
def process_and_speak(frame, run_count, language, book_mode,
    ...
    try:
        ...
        with open(log_path, "a", encoding="utf-8") as f_log:
            ...
            f_log.write("CODE VERSION: pipeline_cli_box4a.py\n")
