"""
pipeline_cli_box3b1_1.py  (box3b1 + Local Offline Model Cache)
============================================================
Copy of pipeline_cli_box3b1.py but configured to load model
weights directly from the local folder 'spelling_model' offline.

Usage:
    python pipeline_cli_box3b1_1.py
"""
...
def load_spell_corrector():
    """Load the T5-based spell correction model from the local directory onto GPU."""
    t0 = time.time()
    local_model_path = os.path.join(PIPELINE_DIR, "spelling_model")
    print(f"[LOAD] Loading spell corrector from local path: {local_model_path}")
...
def process_and_speak(frame, run_count, language, book_mode,
    ...
    try:
        ...
        with open(log_path, "a", encoding="utf-8") as f_log:
            ...
            f_log.write(f"CODE VERSION: pipeline_cli_box3b1_1.py\n")
...
BANNER = r"""
╔══════════════════════════════════════════════════════════════╗
║  FYDP SMART GLASSES — BOX3B1_1 (T5 Spell Local Cache)      ║
