"""
pipeline_cli_box3d1.py  (Box3d + Large-Font Fixes)
==================================================================================
Based on pipeline_cli_box3d.py with fixes for 16/18pt font captures:
  ...
  3. INDENT-BASED PARAGRAPH DETECTION: Detects first-line indents as paragraph
     boundaries — standard typeset paragraph starts are recognized.
  4. TIGHTER GAP CEILING: Reduced paragraph_gap_limit ceiling from 1.35x to
     1.10x median_height so large-font inter-paragraph gaps trigger splits.
  5. All other features from box3d are preserved unchanged.

Usage:
    C:/Users/ali/Desktop/FYDP/fydp/Scripts/python.exe pipeline_cli_box3d1.py
"""
...
def group_ocr_lines_into_paragraphs(lines, image_shape):
    ...
    [BOX3D1] Added first-line indent detection: if a line's left edge is
    significantly indented past the column's left margin, it starts a new
    paragraph.  Also tightened the gap ceiling from 1.35x to 1.10x for
    large-font support.
    """
    if not lines:
        return []
    ...
    # [BOX3D1] Reduced ceiling from 1.35 to 1.10 so large inter-paragraph
    # gaps in big fonts are not absorbed into one paragraph.
    paragraph_gap_limit = min(
        max(0.80 * median_height, 2.20 * typical_gap),
        1.10 * median_height)
    heading_height = 1.25 * median_height

    # [BOX3D1] Compute column left margin for indent detection.
    # The 15th percentile of all line left-edges gives the "normal" left margin,
    # ignoring a few outlier lines that may be indented or offset.
    left_edges = sorted(line["bbox"][0] for line in ordered)
    column_left_margin = float(left_edges[max(0, len(left_edges) * 15 // 100)])
    # Indent threshold: ~0.8x the median line height (roughly 2 character widths)
    indent_threshold = 0.8 * median_height
    ...
    def is_indented(line):
        """[BOX3D1] Return True if the line's left edge is indented past the
        column margin — a standard typographic paragraph-start signal."""
        return (line["bbox"][0] - column_left_margin) > indent_threshold
    ...
    for current in ordered[1:]:
        ...
        # [BOX3D1] First-line indent detection: if the current line is
        # indented and the previous line ended with sentence-ending punctuation,
        # this is very likely a new paragraph.
        elif (is_indented(current) and not is_indented(previous)
              and previous["text"].rstrip()[-1:] in '.?!:'):
            same_paragraph = False
...
def detect_text_region_size(frame, ocr_engine):
    ...
    touching_edges = 0
    # [BOX3D1] Reduced from 3.5% to 1.5% so large-font text near page edges
    # doesn't permanently trigger PAGE CUT OFF.
    edge_margin = int(0.015 * w_orig)
    ...
    # [BOX3D1] Percentage-based edge check: only flag as cut off if >15%
    # of detected text boxes touch the edge.  One or two stray boxes near
    # the edge is normal for a page filling the frame at large font sizes.
    edge_touch_ratio = touching_edges / max(1, num_lines)
    page_touching_edge = edge_touch_ratio > 0.15
...
def process_and_speak(frame, run_count, language, book_mode,
    ...
    try:
        ...
        with open(log_path, "a", encoding="utf-8") as f_log:
            ...
            f_log.write("CODE VERSION: pipeline_cli_box3d1.py\n")
...
BANNER = r"""
╔══════════════════════════════════════════════════════════════╗
║   FYDP SMART GLASSES — BOX3D1 LARGE-FONT PIPELINE          ║
