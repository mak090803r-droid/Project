"""
pipeline_nllb3.3b.py
====================
Integrated OCR → Translation → TTS pipeline.
Uses NLLB-200 3.3B (via CTranslate2 pre-compiled) for translation.

Usage:
    python pipeline_nllb3.3b.py
...
# Pre-compiled 3.3B model from community hub
NLLB_CT2_PRECOMPILED = "michaelfeil/ct2fast-nllb-200-3.3B"
...
def load_nllb_translator(language: str):
    """
    Load NLLB-200 3.3B model via pre-compiled CTranslate2 snapshot.
    Returns (translator, tokenizer) or (None, None) for English.
    """
    if language == "english":
        return None, None  # no translation needed

    print(f"[LOAD] Loading NLLB-200 3.3B Flagship translator …")
    t = time.time()

    # Download or load from local cache
    print(f"[INFO] Loading Pre-Compiled 3.3B Transformer files …")
    try:
        model_dir = snapshot_download(repo_id=NLLB_CT2_PRECOMPILED, local_files_only=True)
    except Exception:
        print("[INFO] Model not found locally. Downloading from Hub (one-time setup) …")
        model_dir = snapshot_download(repo_id=NLLB_CT2_PRECOMPILED, local_files_only=False)

    # Initialize CTranslate2 translator
    print("[INFO] Initializing optimized 3.3B layers into CTranslate2 execution space …")
    translator = ctranslate2.Translator(model_dir, device=NLLB_DEVICE, compute_type=NLLB_COMPUTE_TYPE)

    # Load tokenizer from the same pre-compiled model directory
    tokenizer = transformers.AutoTokenizer.from_pretrained(model_dir)

    print(f"[LOAD] NLLB 3.3B translator ready in {time.time()-t:.2f}s")
...
def run_translation_nllb(text: str, language: str, translator, tokenizer) -> tuple[str, float]:
    """
    Translate text from source language → English using NLLB 3.3B via CTranslate2.
    ...
    print("[STAGE 2] Translating to English (NLLB 3.3B) …")
    t0 = time.time()
...
║          Smart Glasses FYDP — NLLB 3.3B Edition             ║
╚══════════════════════════════════════════════════════════════╝
"""
...
def main():
    ...
    print(f"  Translator : NLLB-200 3.3B Flagship (CTranslate2)")
    print("=" * 60)
    ...
    while True:
        ...
        # ── Timing Summary ─────────────────────────────────────────
        print(f"\n{'─'*40}")
        print("  ⏱  TIMING BREAKDOWN (NLLB 3.3B)")
        print(f"{'─'*40}")
